<div>
    <form class="row g-3 mt-3" wire:submit.prevent="update">
        ...
        <div class="col-md-6">
            ...
            <input type="text" wire:model="nama_product" class="form-control @error('nama_product') is-invalid @enderror" id="inputAddress" placeholder="Contoh: BUKU" required>
            @error('nama_product') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        <div class="col-md-6">
            <label for="inputAddress" class="form-label">Jenis Produk</label>
            <input type="text" wire:model=jenis_product class="form-control @error('jenis_product') is-invalid @enderror" id="inputAddress" placeholder="Contoh: ALAT TULIS" required>
            @error('jenis_product') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        {{-- <div class="col-md-6">
